...
import { fetchAddNominal } from "../../redux/nominal/action";
...
const AddNominal = () => {
    ...
    const loadingStatus = useSelector((state) => state.nominalReducer.loading);

    const onSubmit = data => {
        if (data !== '') {
            uploadImage(img).then(message => {
                const newIcon = message.response.data.url;
                dispatch(fetchAddNominal(token, title, description, newIcon))
            })
            .catch(error => {
