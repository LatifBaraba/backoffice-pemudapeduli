...
import { fetchEditKabarTerbaruRutin } from "../../redux/kabarterbaru/action";
import { Form, Row, Col, Container } from "react-bootstrap";
import { fetchPaket } from "../../redux/paket/action";
import { addCommas, removeNonNumeric } from "../../helper/index";

const EditKabarTerbaruRutin = (props) => {
  ...
  const [id_pp_cp_program_donasi_rutin, setIdProgramDonasiRutin] = useState(data.id_pp_cp_program_donasi_rutin);
  ...
  const donasiRutinData = useSelector((state) => state.paketReducer.paket);
  ...
  return (
    <Fragment>
      <Breadcrumb title="Kabar Terbaru Rutin Page" parent="Dashboard" />
      <div className="container-fluid">
        <div className="row">
          <div className="col-sm-12">
            <div className="card">
              <div className="card-header">
                <h5>Edit Kabar Terbaru Rutin</h5>
              </div>
                    ...
                    <div className="col-md-6 col-sm-12">
                      <div className="form-row">
                        <div className="col-md-12 mb-3">
                          <label>{"Pilih Donasi Rutin"}</label>
                          <Form.Group controlId="formProgramDonasiRutin">
                            <Form.Control
                              required
                              as="select"
                              type="select"
                              onChange={(e) => setIdProgramDonasiRutin(e.target.value)}
                              disabled
                            >
                              <option value="">---Pilih Donasi Rutin---</option>
                              {donasiRutinData.map((donasiRutin, index) =>
                                donasiRutin.id == id_pp_cp_program_donasi_rutin ? (
                                  <option
                                    key={index}
                                    defaultValue={donasiRutin.id}
                                    selected
                                  >
                                    {donasiRutin.title}
                                  </option>
                          ...
                          <span>{errors.name && "ID Donasi Rutin is required"}</span>
...
export default EditKabarTerbaruRutin;
